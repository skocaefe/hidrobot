...
with tabs[0]:
    ...
    with col1:
        ...
        st.session_state.a_tank = st.number_input("A Tankı Hacmi (litre):", min_value=1, max_value=1000,
                                                  value=st.session_state.a_tank)
        st.session_state.b_tank = st.number_input("B Tankı Hacmi (litre):", min_value=1, max_value=1000,
                                                  value=st.session_state.b_tank)
        st.session_state.konsantrasyon = st.number_input("Konsantrasyon Oranı:", min_value=1, max_value=1000,
    ...
    with col2:
        ...
        with col_a:
            ...
            for ion in ["NO3", "H2PO4", "SO4"]:
                st.session_state.recete[ion] = st.number_input(
                    f"{ion}:", value=float(st.session_state.recete[ion]), min_value=0.0, max_value=30.0, step=0.1,
                    format="%.2f", key=f"{ion}_input"
                )
        with col_b:
            st.subheader("Katyonlar (mmol/L)")
            for ion in ["NH4", "K", "Ca", "Mg"]:
                st.session_state.recete[ion] = st.number_input(
                    f"{ion}:", value=float(st.session_state.recete[ion]), min_value=0.0, max_value=20.0, step=0.1,
                    format="%.2f", key=f"{ion}_input"
                )
            ...
            with col:
                for element in elements:
                    st.session_state.recete[element] = st.number_input(
                        f"{element}:", value=float(st.session_state.recete.get(element, 0.0)), min_value=0.0,
                        max_value=100.0, step=0.1, format="%.1f", key=f"{element}_input"
                    )
        ...
        with col_denge1:
            anyon_df = pd.DataFrame(
                [[ion, st.session_state.recete[ion], st.session_state.recete[ion] * abs(iyon_degerlikleri[ion])] for ion
                 in ["NO3", "H2PO4", "SO4"]],
        ...
        if fark < 0.5:
            ...
            st.markdown("**İyileştirme Önerisi:** " + (
                "Anyon fazlası var. Daha fazla katyon ekleyin." if anyon_toplam > katyon_toplam else "Katyon fazlası var. Daha fazla anyon ekleyin."))
        ...
        with col:
            ...
            for ion in ions:
                st.session_state.kuyu_suyu[ion] = st.number_input(
                    f"{ion}:", value=float(st.session_state.kuyu_suyu[ion]), min_value=0.0, max_value=10.0, step=0.05,
                    format="%.2f", key=f"kuyu_{ion}_input"
                )
    ...
    with col2:
        ...
        for element, gubreler_listesi in mikro_element_gruplari.items():
            ...
            secilen_gubre = st.radio(
                ...
                index=0 if st.session_state.secilen_mikro_gubreler[
                               element] not in gubreler_listesi else gubreler_listesi.index(
                    st.session_state.secilen_mikro_gubreler[element]) + 1,
...
def hesapla_mikro_besinler(recete, secilen_mikro_gubreler, konsantrasyon, b_tank_hacmi):
    ...
    for element, label in [("Fe", "Demir"), ("B", "Bor"), ("Mn", "Mangan"), ("Zn", "Çinko"), ("Cu", "Bakır"),
                           ("Mo", "Molibden")]:
