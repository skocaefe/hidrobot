️ Aşağıdaki besinler reçete ihtiyacından fazla eklendi:")
...
def main():
    ...
    with tabs[2]:
        ...
        with st.expander("Hata Ayıklama: Gübre Durumu"):
            ...
            st.write(f"**Tüm Kullanılabilir Gübreler Durumu:** {st.sessionimport streamlit as st
import pandas as pd
import numpy as np
...
# Başlangıç session state ayarları
...
def gubre_hesapla():
    ...
    with tabs[3]:
        ...
        if st.button("Gübre Hesapla", type="primary"):
            # Hesaplamayı yap ve sonucu kaydet
            st.session_state.hesaplama_sonucu = gubre_hesapla()
            
            # Hesaplama sonucunu göster
...
# Hesaplama sonucunu gösterme fonksiyonu
def goster_hesaplama_sonucu(hesaplama_sonucu):
    ...
    with col_sonuc1:
        ...
        if hesaplama_sonucu["a_tank_sonuc"]:
            a_tank_df = pd.DataFrame(hesaplama_sonucu["a_tank_sonuc"], columns=["Gübre Adı", "Formül", "mmol/L", "mg/L", "kg/Tank"])
            st.dataframe(a_tank_df.style.format({"mmol/L": "{:.2f}", "mg/L": "{:.2f}", "kg/Tank": "{:.3f}"}))
            st.write(f"**Toplam A Tankı gübresi:** {hesaplama_sonucu['a_tank_toplam']/1000:.3f} kg")
        else:
    ...
    with col_sonuc2:
        ...
        if hesaplama_sonucu["b_tank_sonuc"]:
            b_tank_df = pd.DataFrame(hesaplama_sonucu["b_tank_sonuc"], columns=["Gübre Adı", "Formül", "mmol/L", "mg/L", "kg/Tank"])
            st.dataframe(b_tank_df.style.format({"mmol/L": "{:.2f}", "mg/L": "{:.2f}", "kg/Tank": "{:.3f}"}))
            st.write(f"**Toplam B Tankı gübresi:** {hesaplama_sonucu['b_tank_toplam']/1000:.3f} kg")
        else:
    ...
    if hesaplama_sonucu["mikro_sonuc"]:
        mikro_df = pd.DataFrame(hesaplama_sonucu["mikro_sonuc"], columns=["Gübre Adı", "Formül", "mikromol/L", "mg/L", "gram/Tank"])
