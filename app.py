...
if st.button("Reçeteyi Hesapla"):
    ...
    if round(anyon_me, 2) != round(katyon_me, 2):
        ...
        if total_nh4_needed > 0:
            ...
            if adjusted_recete["H2PO4"] > 0:
                # MAP ile hem NH₄ hem de H₂PO₄ ihtiyacını karşılamaya çalış
                nh4_from_map = min(total_nh4_needed, adjusted_recete["H2PO4"] / (0.266 / 0.17))  # MAP'in NH₄ katkısı
        ...
        if adjusted_recete["Ca"] > 0:
            gubre_miktarlari_mmol["Kalsiyum Nitrat"]["Ca"] = adjusted_recete["Ca"]
            gubre_miktarlari_mmol["Kalsiyum Nitrat"]["NO3"] = adjusted_recete["Ca"] * (0.144 / 0.187) * (62 / 14)
        ...
        remaining_no3 = adjusted_recete["NO3"] - (gubre_miktarlari_mmol["Kalsiyum Nitrat"]["NO3"] + gubre_miktarlari_mmol["Magnezyum Nitrat"]["NO3"])
        if remaining_no3 > 0:
        ...
        if total_stok_a_kg > tank_a_capacity_kg or total_stok_b_kg > tank_b_capacity_kg:
            ...
            suggested_konsantrasyon_a = sum(gubre_miktarlari_gram.get(gubre, 0) for gubre in ["Kalsiyum Nitrat", "Magnezyum Nitrat", "Kalsiyum Hidroksit", "Calmag"]) / max_stok_a_kg if max_stok_a_kg > 0 else konsantrasyon
            suggested_konsantrasyon_b = sum(gubre_miktarlari_gram.get(gubre, 0) for gubre in gubre_miktarlari_gram if gubre not in ["Kalsiyum Nitrat", "Magnezyum Nitrat", "Kalsiyum Hidroksit", "Calmag"]) / max_stok_b_kg if max_stok_b_kg > 0 else konsantrasyon
        ...
        tablo_gubre = {
            "Kimyasal Bileşik": ["Amonyum Sülfat", "Kalsiyum Nitrat", "Magnezyum Nitrat", "Mono Amonyum Fosfat", "Mono Potasyum Fosfat", "Potasyum Nitrat", "Potasyum Sülfat", "Demir-EDDHA", "Boraks", "Mangan Sülfat", "Çinko Sülfat", "Bakır Sülfat", "Sodyum Molibdat", "TOPLAM"],
            "mmol/L": [
                gubre_miktarlari_mmol["Amonyum Sülfat"]["NH4"],
            ...
            "SO4": [
                gubre_miktarlari_mmol["Amonyum Sülfat"]["SO4"],
                "-", "-", "-",
                "-", "-",
            ...
            "Gram (1000 L)": [
                f"{gubre_miktarlari_gram.get('Amonyum Sülfat', 0.0):.2f}",
